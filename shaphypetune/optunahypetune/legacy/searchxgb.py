...
def regularize_metric(eval_metric):
    ...
    for metric in eval_metric:
        if metric == 'f1':
            ...
            # but for built in gbdt_utils, xgb will automatically decide
...
class Objective(object):
    def __init__(self,
        ...
        eval_metric_list: a list of str, gbdt_utils for monitor
...
class GridSearchXGB(object):
    ...
    def predict_proba_with_best_threshold(self, x, y, metric_list=['f1']):
        ...
        :param metric_list: a list of gbdt_utils
        """
        max_score_list = []
        best_t_list = []
    ...
    def _regularize_metric(self, eval_metric, early_stopping_rounds):
        ...
        if early_stopping_rounds is not None:
            ...
            for metric in eval_metric:
                if metric == 'f1':
                    # when this happens, feval will always be used for early stopping.
                    # For custom function/gbdt_utils not like 'auc***', the smaller the returned value the better,
                    # but for built in gbdt_utils, the cv package will automatically decide
                    # according to the metric type, e.g., the smaller the better for error
                    # but the bigger the better for auc, etc.
